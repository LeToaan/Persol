...
class LensesController extends Controller
{
    public function index()
    {
        ...
        // ->get()
        // ;
        return view('admin/lenses/index', ['data1' => $a]);
    }
    public function delete($id)
    {
        ...
        // DB::table('product_properties_lenses')->where('product_id', $id)->delete();
        ...
        return redirect()->route('admin.product.lenses.index')->with('Success', $success);
    }
    public function create(Request $request)
    {

        $brand = DB::table('brand')->where('category_brand', 'lenses')->get();
        $country = DB::table('country')->get();
        $category = DB::table('category_product')->where('parent_category', '2')->get();

        //    ['category'=>$category],['country'=>$country],['brand'=>$brand]

        return view('admin/lenses/create', compact('category', 'brand', 'country'));
    }
    public function store(Request $request)
    {
        ...
        $properties = $request->only('color','extract','coating','special','material');

        $properties['product_id'] = $idProduct->id_product;
        //in dữ liệu zoo bảng properties
        DB::table('product_properties_lenses')->insert($properties);
        ...
        return redirect()->route('admin.product.lenses.create')->with('Success',$success);
    }
    public function edit($id)
    {
        ...
        $properties = DB::table('product_properties_lenses')->where('product_id', $id)->first();
        //vitri để gán giá trị vô chỗ id="vitri" để add thêm image
        $vitri = DB::table('image_product')->where('product_id', $id)->get()->count();

        return view('admin/lenses/edit', compact('brand', 'country', 'category', 'image', 'vitri', 'product', 'properties'));
    }
    public function update(Request $request, $id)
    {
        ...
        $productList = "name_product,featured_product,
        content_product,category_id,brand_id,country_id,price_product";
        $propertiesList = 'color,extract,coating,special,material';
        $product = $request->only('name_product', 'sale_product' ,'featured_product', 'content_product', 'category_id', 'brand_id', 'country_id', 'price_product');
        ...
        //in dữ liệu zoo bảng properties
        DB::table('product_properties_lenses')->where('product_id', $id)->update($properties);
        $vitri = $request->input('vitri');
        if(!empty($request->file('image_product'))){
        foreach ($request->file('image_product') as $key1 => $file) {
    ...
    {   
        ...
        return view('admin/lenses/detail', compact('product','image','properties'));
    }
}
